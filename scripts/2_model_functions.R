...
fun_SP <- function(C, M, CoS, PoR, lambda1, lambda2, lambda3, lambda4, i) {
  
  value_smok <- sum((lambda1 * C[i]) - (lambda2 * M[i]) - (lambda3 * CoS[i]))
  value_no_smok <- sum((lambda2 * M[i]) - (lambda1 * C[i]))
  value_reg <- sum((lambda2 * M[i]) - (lambda1 * C[i]) - (lambda4 * PoR[i]))
...
simulate_data <- function(params) {
  ...
  CoS <- rnorm(params$nTime, mean = 3, sd = 3)
  ...
  for (i in 1:params$nTime) {
    ...
    result <- fun_SP(C = C, M = M, CoS = CoS, PoR = PoR, lambda1 = params$lambda1, lambda2 = params$lambda2, lambda3 = params$lambda3, lambda4 = params$lambda4, i = i)
  ...
  st_t <- st_df %>%
    mutate(timepoint = row_number()) %>%  # create a timepoint column based on row number
  ...
  sim_df <- data.frame(nTime = 1:params$nTime, impulse, LAP, switch_t, NW, st_df, st_t, S, ES, CC, CR, C, transformed_PE, SE, M)
  
  return(sim_df)
  
}

# to test that the function is working, remove commented out aspect of below section and run entire script
